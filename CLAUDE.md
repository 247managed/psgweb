...
## Pre-filled Release of Information

BHIS referral emails carry a PDF of the practice's "Authorization to Obtain or Release
Health Care Information", pre-filled from the referral so staff do not retype it.
Generated by `buildReleaseHtml()` in `apps-script/Code.gs` and converted with
`Utilities.newBlob(html, 'text/html').getAs('application/pdf')` — no external service.

**It is intentionally not a completed authorization.** The signature, date, expiration,
record-type checkboxes, the federal specific-consent initials, and SS# are all left blank,
because referrals usually come from school staff or case managers who cannot sign a release
on the family's behalf. The referrer's typed referral signature is never reused as the
authorizing signature. This is marked in a banner on the PDF, in its filename, and in the
email body.

- Pre-fill values are looked up by the `key` on each submitted field (the input's `name`),
  not by display label — see `byKey()`. Renaming an input's `name` breaks a pre-fill; renaming
  its `data-label` does not.
- Turn it off with `CONFIG.attachReleaseForm = false`.
- If PDF generation fails the referral email still sends, without the attachment, and the
  body tells staff to use the paper form.
- The practice address block and the stated purpose live in `CONFIG.practice` and
